 ...
 */
export default function LineReveal({ children, delay = 0, className = '', inline = false, style = {}, as: Tag = 'span', ...rest }) {
  const wrapClass = `reveal-line-wrap ${inline ? 'reveal-inline' : ''} ${className}`;
  const customStyle = { ...style, '--line-delay': `${delay}s` };

  return (
    <Tag className={wrapClass} style={customStyle} {...rest}>
 ...
 */
export function RevealItem({ children, delay = 0, className = '', style = {}, as: Tag = 'div', ...rest }) {
  const customStyle = { ...style, '--line-delay': `${delay}s` };

  return (
    <Tag className={`reveal-item-wrap ${className}`} style={customStyle} {...rest}>
      {children}
    </Tag>
  );
}
